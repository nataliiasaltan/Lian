...
jQuery(document).ready(function($) {
    $(window).scroll(function() {
        if ($(window).scrollTop() > 100) {
            $('.logochange').each(function() { this.src = this.getAttribute('pavel-mavel'); });
        } else {
            $('.logochange').each(function() { this.src = this.removeAttribute('pavel-mavel'); });
        }
    });
});